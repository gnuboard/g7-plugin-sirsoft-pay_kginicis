...
- KG 이니시스 간편결제 결제수단을 주문서에서 브랜드 아이콘과 짧은 설명으로 표시할 수 있는 `easy_pay_show_brand_button` 설정을 추가했습니다.

### Fixed

- 주문서에서 KG 이니시스 간편결제 결제수단 텍스트가 데이터 로딩 후 늦게 채워지는 경우 브랜드 버튼 표시가 적용되지 않거나 1020px 전후 화면에서 문구가 버튼 밖으로 벗어날 수 있던 문제를 수정했습니다. 좁은 카드 폭에서는 제목과 설명이 총 2줄로 유지되도록 보정했습니다.
...
- 비-JPY 주문(KRW 등)으로 진행하는 주문서에서 일본 전용 결제수단(PayPay, 일본 편의점결제)을 결제수단 목록에서 자동으로 숨기도록 변경했습니다. 기존 JPY 주문에서 일본 외 결제수단을 차단하던 정책과 양방향으로 동작합니다.
...
- KG 이니시스 PC 결제창 호출 중 이니시스 안내 팝업/빈 모달 iframe만 남는 경우 임시 주문이 `pending_order` / `ready`로 남지 않고 결제취소 상태로 기록되도록 보강했습니다.
- KG 이니시스 간편결제 결제창 닫힘 보고 시 선택한 간편결제 수단을 결제 메타에 보존하도록 수정했습니다.
